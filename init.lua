...
require("lazy").setup({
  ...
  },
  {
    ...
    config = function()
      require("mason-null-ls").setup({
        ensure_installed = { "prettier", "isort", "black", "stylua" },
      })
      vim.keymap.set({ "n", "v" }, "<leader>mf", vim.lsp.buf.format, { desc = "[m]ake [f]ormatted" })
    end,
  },
  ...
  -- Highlight unique character to move faster with f/F
  {
    "jinh0/eyeliner.nvim",
    config = function()
  ...
  -- nvim-tree for directory Navigation
  {
  ...
  -- Plugin for preview plantuml diagrams
  {
  ...
  -- Plugin for Debug Adapter protocol client
  {
    ...
    config = function()
      ...
      vim.keymap.set("n", "<leader>do", "<cmd>lua require'dap'.step_over()<CR>", { silent = true })
...
vim.o.tabstop = 4
vim.o.shiftwidth = 4
-- Set Columns limiter for max line length
